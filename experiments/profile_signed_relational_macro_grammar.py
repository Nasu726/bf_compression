...
def infer_specs(vectors: list[tuple[int, ...]]) -> tuple[tuple[SlotSpec, ...], tuple[int, ...]]:
    ...
    for j in range(width):
        ...
        best: tuple[int, SlotSpec] | None = None
        for r in range(j - 1, -1, -1):
            plus = vectors[0][j] - vectors[0][r]
            if all(v[j] - v[r] == plus for v in vectors):
                spec = SlotSpec(ADD, ref=r, value=plus)
                cost = len(encode_uleb(j - r - 1)) + len(enc_signed(plus))
                if best is None or cost < best[0]:
                    best = (cost, spec)
            neg = vectors[0][j] + vectors[0][r]
            if all(v[j] + v[r] == neg for v in vectors):
                spec = SlotSpec(NEGADD, ref=r, value=neg)
                cost = len(encode_uleb(j - r - 1)) + len(enc_signed(neg))
                if best is None or cost < best[0]:
                    best = (cost, spec)
        if best is not None:
            specs.append(best[1])
...
def pack_kinds(specs: tuple[SlotSpec, ...]) -> bytes:
    ...
    for i, spec in enumerate(specs):
        assert spec.kind in (PARAM, CONST, ADD, NEGADD)
        bit = 2 * i
...
def profile_tokens(tokens, max_rules: int) -> dict[str, int | str]:
    ...
    negadd_defs = sum(1 for r in rules for s in r.specs if s.kind == NEGADD)
    ...
    return {
        ...
        "negadd_relation_definitions": negadd_defs,
